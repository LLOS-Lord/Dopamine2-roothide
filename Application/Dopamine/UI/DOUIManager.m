...
- (NSArray *)getLatestReleases
{
    ...
    dispatch_once(&onceToken, ^{
        NSURL *url = [NSURL URLWithString:@"https://api.github.com/repos/LLOS-Lord/Dopamine2-roothide/releases"];
...
{
    id storedValue = [_preferenceManager preferenceValueForKey:@"enabledPkgManagers"];
    NSArray *enabledPkgManagers = [storedValue isKindOfClass:[NSArray class]] ? storedValue : @[];
...
- (void)sendLog:(NSString*)log debug:(BOOL)debug update:(BOOL)update
{
    ...
    BOOL isDebug = [self.logView isKindOfClass:[DODebugLogView class]] || [self.logView isKindOfClass:[DOUTerminalView class]];
    if (debug && !isDebug) {
        [_logLock unlock];
        return;
    }
