...
/********************** roothide *************************/
        ...
        if (!_systemInfoXdict) {
            ...
            NSString *message = xpfError ? [NSString stringWithFormat:@"XPF failed with error: (%s)", xpfError] : @"XPF failed to build the kernel offset dictionary";
            xpf_stop();
...
- (NSError *)doExploitation
{
    ...
    if ([kernelExploit load] != 0) return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedLoadingExploit userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Failed to load kernel exploit: %@", kernelExploit.loadError ?: @"Unknown dyld load error"]}];
    ...
    if (pacBypass) {
        ...
        if ([pacBypass load] != 0) {[kernelExploit cleanup]; return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedLoadingExploit userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Failed to load PAC bypass: %@", pacBypass.loadError ?: @"Unknown dyld load error"]}];};
    ...
    if (shouldRunPPLBypass) {
        ...
        if ([pplBypass load] != 0) {[pacBypass cleanup]; [kernelExploit cleanup]; return [NSError errorWithDomain:JBErrorDomain code:JBErrorCodeFailedLoadingExploit userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Failed to load PPL bypass: %@", pplBypass.loadError ?: @"Unknown dyld load error"]}];};
    ...
    if (!gPrimitives.kalloc_global) {
        if (@available(iOS 17.0, *)) {
            // TXM-backed kernels cannot safely write to page-table allocations.
            // Refuse this path instead of reproducing the random panic fixed in
            // RootHide d986214; iOS 16 keeps the legacy fallback below.
            [pplBypass cleanup];
            [pacBypass cleanup];
            [kernelExploit cleanup];
            return [NSError errorWithDomain:JBErrorDomain
                                       code:JBErrorCodeFailedExploitation
                                   userInfo:@{NSLocalizedDescriptionKey : @"IOSurface kalloc is unavailable on iOS 17+; refusing the unsafe page-table allocator"}];
        }
        // iOS 16 legacy path: use leaked page tables when IOSurface kalloc is unavailable.
...
- (NSError *)injectLaunchdHook
{
    ...
    // A failed launchd port stash leaves registeredPorts[2] null on iOS 18.
    // Do not continue into opainject and create a half-injected launchd state.
...
/******************************** roothide specific *************************/
    ...
    *errOut = [self finalizeBootstrapIfNeeded];
    ...
    // Package postinst scripts normally refresh their own icons, but the
    // first RootHide bootstrap can run while iconservicesagent is restarting.
    // Refresh once after all bundled packages are installed so Sileo and
    // RootHide Manager are registered before the userspace reboot.
    [[DOUIManager sharedInstance] sendLog:DOLocalizedString(@"Refreshing Jailbreak Apps") debug:NO];
