...
{
    CFPropertyListRef productVersion = MGCopyAnswer(CFSTR("ProductVersion"));
    if (!productVersion) return nil;
    return CFBridgingRelease(productVersion);
}
...
- (NSString *)versionSupportString
{
    ...
    if ([self isArm64e]) {
        if (![self isSPTM]) return @"iOS 15.0 - 17.3.1 (PPL)";
...
- (NSString *)accessibleKernelPath
{
    if ([self isInstalledThroughTrollStore] || getuid() == 0) {
        ...
        NSDictionary *attributes = [fileManager attributesOfItemAtPath:kernelcachePath error:nil];
        unsigned long long kernelcacheSize = [attributes[NSFileSize] unsignedLongLongValue];
        BOOL hasUsableKernelcache = attributes != nil && kernelcacheSize > (1024ULL * 1024ULL);
        if (!hasUsableKernelcache) {
            if (attributes != nil) [fileManager removeItemAtPath:kernelcachePath error:nil];
            NSString *documentsPath = [NSHomeDirectory() stringByAppendingPathComponent:@"Documents"];
            if (grab_images(documentsPath) == false) return nil;
            NSDictionary *downloadedAttributes = [fileManager attributesOfItemAtPath:kernelcachePath error:nil];
            if ([downloadedAttributes[NSFileSize] unsignedLongLongValue] <= (1024ULL * 1024ULL)) {
                [fileManager removeItemAtPath:kernelcachePath error:nil];
                return nil;
            }
        }
        return kernelcachePath;
...
- (NSString *)accessibleSPTMPath
{
    ...
    NSArray<NSString *> *candidates = @[
        [NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:@"sptm.img4"],
        [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sptm.img4"],
        [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/sptm.im4p"]
    ];
...
- (NSString *)accessibleTXMPath
{
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSArray<NSString *> *candidates = @[
        [NSBundle.mainBundle.bundlePath stringByAppendingPathComponent:@"txm.img4"],
        [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/txm.img4"],
        [NSHomeDirectory() stringByAppendingPathComponent:@"Documents/txm.im4p"]
    ];
    for (NSString *path in candidates) {
        if ([fileManager fileExistsAtPath:path]) return path;
    }
...
- (NSError *)updateBootLogo
{
    ...
    BOOL bootLogoEnabled = [[DOPreferenceManager sharedManager] boolPreferenceValueForKey:@"bootlogoEnabled" fallback:YES];
    BOOL customBootLogoEnabled = [[DOPreferenceManager sharedManager] boolPreferenceValueForKey:@"customBootlogoEnabled" fallback:NO];

    UIImage *bootLogoImage = nil;
    if (bootLogoEnabled && customBootLogoEnabled) {
        bootLogoImage = [UIImage imageWithContentsOfFile:[DOUIManager sharedInstance].bootlogoPath];
    }

    // A missing/default logo must never block jailbreak. The built-in renderer
    // is intentionally not used in the critical bootstrap path; a custom logo
    // is optional and is written only after it has been validated and encoded.
    if (!bootLogoImage || !bootLogoImage.CGImage) {
        NSLog(@"Dopamine: no valid custom boot logo; removing stale logo and continuing");
        [self runAsRoot:^{
            [self runUnsandboxed:^{
                unlink(bootLogoPath);
            }];
    ...
    if (!bootLogoData || bootLogoData.length == 0) {
        NSLog(@"Dopamine: unable to encode custom boot logo; removing stale logo and continuing");
        [self runAsRoot:^{
            [self runUnsandboxed:^{
                unlink(bootLogoPath);
            }];
