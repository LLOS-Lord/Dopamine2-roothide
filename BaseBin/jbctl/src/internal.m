...
int jbctl_handle_internal(const char *command, int argc, char* argv[])
{
	if (!strcmp(command, "launchd_stash_port")) {
		...
		// Do not call mach_ports_lookup on the remote launchd task here.
		// On newer iOS versions that path can fault while copying the
		// remote init-port array. Register the three-port layout directly.
