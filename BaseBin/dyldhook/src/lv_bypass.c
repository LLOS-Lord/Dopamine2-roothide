...
CS_SuperBlob *load_signature(int fd, const fsignatures_t *fs)
{
	...
	vm_address_t addr;
	kern_return_t kr = vm_allocate(task_self_trap(), &addr, superblobSize, VM_FLAGS_ANYWHERE);
	if (kr != KERN_SUCCESS) return NULL;

	lseek(fd, superblobStart, SEEK_SET);
	int r = read(fd, (void *)addr, superblobSize);
	if (r != superblobSize) {
		vm_deallocate(task_self_trap(), addr, superblobSize);
		return NULL;
	}
				...
				if (arg1) {
					...
					if (jbinfo_should_force_cs_adhoc()) {
						...
						if (superblob) {
								...
								if (superblob_find_cdflags_and_team_id(superblob, &cdFlagsOffset, &teamIdOffset) == 0) {
									...
									if (!!(OSSwapBigToHostInt32(*cdFlagsPtr) & CS_ADHOC) == hasTeamId) {
										...
										if (!isFile) {
											...
											vm_address_t addr = 0;
											if (vm_allocate(task_self_trap(), &addr, siginfo.signature.fs_blob_size, VM_FLAGS_ANYWHERE) == 0) {
							...
							if (superblobNeedsFree) {
								vm_deallocate(task_self_trap(), (vm_address_t)superblob, siginfo.signature.fs_blob_size);
							}
						}
