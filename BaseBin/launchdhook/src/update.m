...
void jbupdate_finalize_stage2(const char *prevVersion, const char *newVersion)
{
        ...
        if (cdhashesCount > 1) {
                ...
                // Use jb_trustcache_add_entries() instead of trustcache_file_upload_with_uuid()
                // to avoid kalloc_type zone-bound panics on iOS 17+ (see common.m for rationale).
                r = jb_trustcache_add_entries(dyldTCFile->entries, dyldTCFile->length);
                if (r != 0) {
                        char msg[4000];
