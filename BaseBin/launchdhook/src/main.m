...
/*********************** roothide specific ********************/
        ...
        if (trustcache_nokcall_is_required()) {
                ...
                if (loaderStatus != 0 || pairStatus != 0) {
                        JBLogError("launchdhook signed trustcache restore failed first-load=%u loader-status=%d pair-status=%d", firstLoad, loaderStatus, pairStatus);
                }
        }
