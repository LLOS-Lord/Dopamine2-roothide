...
NSString* resolveLoaderExecutablePaths(NSString *loadPath, NSString *loaderPath, NSString *mainExecutablePath)
{
	...
	if ([loadPath hasPrefix:@"@executable_path/"] || [loadPath isEqualToString:@"@executable_path"]) {
		if (!mainExecutablePath) return nil;
...
{
@autoreleasepool {
	if (!loadPath || !mainExecutablePath || !rpathStack) return nil;
...
{
@autoreleasepool {
	if (!loadPath || !loaderPath || !mainExecutablePath || !fileCaches || !rpathStack || !preferredArch || !cdhashesOut || !cdhashCountOut) return;
	...
	// Always walk dependencies for ad-hoc binaries. A trustcached binary can
	// still reference a dependency installed or replaced after it was trusted.
	if (!isAdhocSigned) {
		fat_free(fat);
		return;
	}
...
{
	if (!path || !cdhashesOut || !cdhashCountOut) return;
