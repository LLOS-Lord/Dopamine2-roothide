    ...
    dispatch_once(&onceToken, ^{
		...
		dispatch_source_set_event_handler(source, ^{
			JBLogDebug("received message from jailbreakd");
				...
				if (err == 0) {
					/* The raw bootstrap port may receive a request before the normal
					 * XPC hook sees it. Handle it instead of aborting launchd. */
...
int initJailbreakd(bool firstLoad)
{
	...
	// Mark initialized only after the child was spawned successfully.
