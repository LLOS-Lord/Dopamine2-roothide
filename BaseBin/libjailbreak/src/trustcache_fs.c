...
void walk_machos_in_dir(const char *dir_path, void (^macho_fat_walk)(const char *path, Fat *fat), bool recurse) {
    ...
    if (!dir) {
        JBLogError("trustcache directory walk failed phase=open path=%s errno=%d", dir_path, errno);
        return;
    }
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        if (lstat(full_path, &st) != 0) {
            JBLogError("trustcache directory walk failed phase=stat path=%s errno=%d", full_path, errno);
            free(full_path);
            continue;
        }
...
void directory_collect_untrusted_cdhashes_by_path(const char *directoryPath,
                                                  bool recursive,
...
int jb_trustcache_add_file(const char *filePath) {
    ...
    if (cdhashes && cdhashCount > 0) {
        ...
        if (status != 0)
            JBLogError("trustcache file publish failed path=%s entries=%u status=%d", filePath, cdhashCount, status);
        free(cdhashes);
        return status;
    }
...
int jb_trustcache_add_directory(const char *directoryPath, bool recursive) {
    ...
    if (cdhashes && cdhashCount > 0) {
        int status = jb_trustcache_add_cdhashes(cdhashes, cdhashCount);
        if (status != 0)
            JBLogError("trustcache directory publish failed path=%s entries=%u status=%d",
                       directoryPath,
                       cdhashCount,
                       status);
        free(cdhashes);
