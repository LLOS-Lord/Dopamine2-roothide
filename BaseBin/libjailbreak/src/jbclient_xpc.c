...
{
        // Deprecated: trustcache clear is now handled by trustcache_nokcall system
        // This function is kept for ABI compatibility but is a no-op
        return 0;
}
...
/* trustcache_nokcall IPC implementations */
int jbclient_root_trustcache_append_entries(const struct trustcache_entry_v1 *entries, uint32_t entryCount)
{
        xpc_object_t xargs = xpc_dictionary_create_empty();
        xpc_dictionary_set_data(xargs, "entries", entries, entryCount * sizeof(struct trustcache_entry_v1));
        xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_ROOT, JBS_ROOT_TRUSTCACHE_APPEND_ENTRIES, xargs);
        xpc_release(xargs);
        if (xreply) {
                int64_t result = xpc_dictionary_get_int64(xreply, "result");
                xpc_release(xreply);
...
int jbclient_platform_trustcache_query_cdhash(const uint8_t cdhash[20], bool *foundOut)
{
        ...
        xpc_object_t xargs = xpc_dictionary_create_empty();
        xpc_dictionary_set_data(xargs, "cdhash", cdhash, 20);
        xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_PLATFORM, JBS_PLATFORM_TRUSTCACHE_QUERY_CDHASH, xargs);
        xpc_release(xargs);
        if (xreply) {
                int64_t result = xpc_dictionary_get_int64(xreply, "result");
                if (result == 0) {
                        *foundOut = xpc_dictionary_get_bool(xreply, "found");
                }
                xpc_release(xreply);
                return result;
        }
...
int jbclient_platform_trustcache_owner_probe(bool *availableOut, int *ownerStatusOut)
{
        ...
        if (ownerStatusOut) *ownerStatusOut = -1;
        xpc_object_t xargs = xpc_dictionary_create_empty();
        xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_PLATFORM, JBS_PLATFORM_TRUSTCACHE_OWNER_STATUS, xargs);
        xpc_release(xargs);
        if (xreply) {
                int64_t result = xpc_dictionary_get_int64(xreply, "result");
                if (result == 0) {
                        if (availableOut) *availableOut = true;
