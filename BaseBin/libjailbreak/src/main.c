...
int jbclient_initialize_primitives_internal(bool physrwPTE)
{
	...
	if (jbclient_root_get_sysinfo(&xSystemInfo) == 0) {
		...
		if (jbclient_root_get_physrw(physrwPTE, &asidPtr) == 0) {
			...
			// The page-table allocator is retained only for the legacy iOS 16
			// path. On iOS 17+ it can produce allocations that TXM cannot write
			// to, causing random panics during dynamic trustcache insertion.
			struct utsname systemInfo = {0};
			bool isPreIOS17 = false;
			if (uname(&systemInfo) == 0) {
				isPreIOS17 = strncmp(systemInfo.release, "23.", 3) < 0;
			}
			if (__builtin_available(iOS 16.0, *) && isPreIOS17) {
