...
int kwritebuf_protected(uint64_t kaddr, const void *input, size_t size)
{
        if (size != 0 && !input)
                return EINVAL;
        if (!gPrimitives.physwritebuf || !gPrimitives.physreadbuf || (!gPrimitives.kvtophys && !gPrimitives.vtophys)) {
                return ENOTSUP;
        }
        if (size == 0)
                return 0;

        void *observed = malloc(size);
        if (!observed)
                return ENOMEM;

        int status = _kwritebuf_phys(kaddr, input, size);
        if (status == 0) {
                status = _kreadbuf_phys(kaddr, observed, size);
                if (status == 0 && memcmp(observed, input, size) != 0) {
                        status = EIO;
                }
        }
        free(observed);
