...
uint64_t _trustcache_list_get_start(void)
{
        if (ksymbol(pmap_image4_trust_caches)) { // iOS <=15
                ...
                uint64_t rtPtr = kread64(ksymbol(ppl_trust_cache_rt) + 0x20);
                if (!rtPtr) return 0;
                return kread64(rtPtr);
        }
        else if (ksymbol_txm(txm_trustcache_root)) { // iOS >=17, SPTM/TXM
                uint64_t rootPtr = kread64(ksymbol_txm(txm_trustcache_root) + 0x20);
                if (!rootPtr) return 0;
                return kread64(rootPtr);
        }
...
void _trustcache_list_set_start(uint64_t newStart)
{
        if (ksymbol(pmap_image4_trust_caches)) { // iOS <=15
                ...
                uint64_t rtPtr = kread64(ksymbol(ppl_trust_cache_rt) + 0x20);
                if (rtPtr) kwrite64(rtPtr, newStart);
        }
        else if (ksymbol_txm(txm_trustcache_root)) { // iOS >=17, SPTM/TXM
                uint64_t rootPtr = kread64(ksymbol_txm(txm_trustcache_root) + 0x20);
                if (rootPtr) kwrite64(rootPtr, newStart);
        }
}
...
int trustcache_list_insert(uint64_t tcToInsert)
{
        ...
        if (ksymbol(SPTMArgs)) {
                ...
                // Never dereference a null list tail. A missing SPTM/TXM trustcache
                // head means the offsets or primitive state are not usable yet.
                if (!lastTC) return -1;
                ...
                if (koffsetof(trustcache, prevptr) && !previousStartTC) return -1;
...
int trustcache_list_remove(uint64_t tcKaddr)
{
        ...
        if (ksymbol(SPTMArgs)) {
                ...
                if (curTc == tcKaddr) {
                        ...
                        uint64_t prevTc = 0;
                        while (curTc != 0 && curTc != tcKaddr) {
                                prevTc = curTc;
                                curTc = kread64(curTc + koffsetof(trustcache, nextptr));
                        }
                        if (curTc == 0) return -1; // not found
...
uint64_t _jb_trustcache_grow(void)
{
        ...
        // Zero the entire allocation first to prevent stale data from causing
        // kalloc_type zone boundary violations on iOS 17+ (SPTM/TXM).
        // kalloc is not guaranteed to return zeroed memory.
        char zeroBuf[0x4000];
        memset(zeroBuf, 0, sizeof(zeroBuf));
        kwritebuf(jbTcKern, zeroBuf, sizeof(zeroBuf));
        ...
        *(uint64_t *)(jbTc->trustcache + koffsetof(trustcache, fileptr)) = (jbTcKern + offsetof(jb_trustcache, file));
        ...
        int insertResult = trustcache_list_insert(jbTcKern);
        if (insertResult != 0) {
                // If insertion fails, free the allocation to avoid a leak
...
int trustcache_file_upload(trustcache_file_v1 *tc)
{
        ...
        int insertResult = trustcache_list_insert(tcKaddr);
