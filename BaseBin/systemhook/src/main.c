...
/*
 * RootHide upstream 7a639ba ("Pretrust complete dpkg tool suite")
 * pretrusts every dpkg helper and APT method before apt/dpkg starts a
 * post-install trigger.  On iOS 16 this prevents a child such as uicache/
 * uikittools from reaching the spawn path without a valid trust entry.
 */
...
static void trust_directory_entries(const char *directoryPath, const char *namePrefix)
{
	...
	while ((entry = readdir(directory)) != NULL) {
		...
		if (snprintf(candidatePath, sizeof(candidatePath), "%s/%s", directoryPath, entry->d_name) >= (int)sizeof(candidatePath)) continue;
...
static void trust_apt_runtime(const char *aptGetPath)
{
	...
	if (!aptGetPath || !string_has_suffix(aptGetPath, aptGetSuffix)) return;

	size_t rootLength = strlen(aptGetPath) - strlen(aptGetSuffix);
	char jbroot[PATH_MAX];
	if (rootLength >= sizeof(jbroot)) return;
	memcpy(jbroot, aptGetPath, rootLength);
	jbroot[rootLength] = '\0';

	char usrBinPath[PATH_MAX];
	if (snprintf(usrBinPath, sizeof(usrBinPath), "%s/usr/bin", jbroot) >= (int)sizeof(usrBinPath)) return;
	trust_directory_entries(usrBinPath, "dpkg");

	char methodsPath[PATH_MAX];
	if (snprintf(methodsPath, sizeof(methodsPath), "%s/usr/libexec/apt/methods", jbroot) >= (int)sizeof(methodsPath)) return;
